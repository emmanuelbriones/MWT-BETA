 ...
 */
if($active_pm == "pm_1_2"){
	$query = "SELECT ra_nonsove,ratio_area, b08301e1 as e1, b08301e3 as e3, ra_publict, ra_walk, ra_bike, pt_nonsove, pt_publict, pt_walk, pt_bike,ST_AsText(SHAPE) as shape FROM pm_1_2 WHERE ST_INTERSECTS( st_geomfromtext( st_astext(@poly), 4326), shape );";
}
	...
	$query = "SELECT OGR_FID,crash_year,type,killed,classA,classB,classC,classO,state,St_astext(SHAPE) as shape FROM pm18_19 as p WHERE  ST_INTERSECTS( st_geomfromtext( st_astext(@poly), 4326), p.SHAPE );";
	...
	$query = "SELECT type,ST_AsText(SHAPE) as shape, w_i_buffer FROM pm20crashes as p WHERE  ST_INTERSECTS( st_geomfromtext( st_astext(@poly), 4326), p.SHAPE );"; 
}
	...
	$query = "SELECT type,state_code,year_recor,iri, PAV_RATING, miles, ST_AsText(SHAPE)  as shape FROM $active_pm as p WHERE  ST_INTERSECTS( st_geomfromtext( st_astext(@poly), 4326), p.SHAPE );"; 
}
	...
	$query = "SELECT type,tactcnt, St_astext(SHAPE) as shape FROM pm4 p WHERE ST_INTERSECTS( st_geomfromtext( st_astext(@poly),4326), p.SHAPE );";
}
